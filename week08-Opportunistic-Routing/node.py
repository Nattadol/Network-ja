...
if __name__ == "__main__":
    # เริ่มต้น Server และลูปสำหรับส่งต่อข้อความ
    threading.Thread(target=start_server, daemon=True).start()
    threading.Thread(target=forward_loop, daemon=True).start()

    # สมมติค่าความน่าจะเป็นเบื้องต้นให้กับ Peers
    for peer in PEER_PORTS:
        delivery_table.update_probability(peer, 0.6)  # ตั้งค่าเป็น 0.6 ซึ่งมากกว่า Threshold (0.5)

    # ทดลองส่งข้อความตอนเริ่มต้น
    for peer in PEER_PORTS:
    ...
    # ลูปหลักเพื่อให้โปรแกรมทำงานต่อไปเรื่อยๆ
    while True:
        time.sleep(1)